...
{
    ui->setupUi(this);
    QString id_str = QString::number(id_paciente);
    ui->txtID->setText(id_str);

    // Realizando uma query que traz os elementos do banco
    QSqlQuery query;
    query.prepare("SELECT * FROM tb_pacientes WHERE id="+QString::number(id_paciente));
...
void edicaopaciente::on_editar_clicked()
{
    ...
    QSqlQuery query;
    QString idString = QString::number(id); // Convertendo o ID para QString
    query.prepare("UPDATE tb_pacientes SET nome = :nome, idade = :idade, cpf = :cpf, contato = :contato, " "email = :email, convenio_plano = :convenio_plano, data_nasc = :data_nasc WHERE id = :id");
